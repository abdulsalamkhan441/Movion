...
const Popular = () => {
  ...
  // Fetch popular movies (Popular Now)
  const fetchPopular = async (pageNum = 1) => {
    if (loading || !TMDB_KEY) return;
    setLoading(true);
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/movie/popular?api_key=${TMDB_KEY}&language=en-US&page=${pageNum}`
      );
    ...
    <>
      ...
      <section className="relative w-full h-screen flex flex-row overflow-hidden">
        <div className="flex-1 md:pl-10 pl-0">
          <SeeMoreHero endpoint="/movie/popular" />
        </div>
      ...
      <section className="bg-black pb-16">
        <NewMovieRow movies={popularMovies} title="Popular Right Now" />
