...
const Fantasy = () => {
  ...
  // Fetch Fantasy movies (discover by genre id 14)
  const fetchFantasy = async (pageNum = 1) => {
    if (loading || !TMDB_KEY) return;
    setLoading(true);
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_KEY}&with_genres=14&language=en-US&page=${pageNum}`
      );
    ...
    <>
      ...
      <section className="relative w-full h-screen flex flex-row overflow-hidden">
        <div className="flex-1 md:pl-10 pl-0">
          <SeeMoreHero endpoint="/discover/movie" params={{ with_genres: 14 }} />
        </div>
      ...
      <section className="bg-black pb-16">
        <NewMovieRow movies={fantasyMovies} title="Fantasy Movies" />
