  ...
  const fetchUpcoming = async (pageNum = 1) => {
    ...
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/movie/upcoming?api_key=${TMDB_KEY}&language=en-US&page=${pageNum}`
      );
    ...
    <>
      ...
      <section className="relative w-full h-screen flex flex-row overflow-hidden">
        <div className="flex-1 md:pl-10 pl-0">
          <SeeMoreHero endpoint="/movie/upcoming" />
        </div>
      ...
      <section className="bg-black pb-16">
        <NewMovieRow movies={upcomingMovies} title="Coming Soon" />
